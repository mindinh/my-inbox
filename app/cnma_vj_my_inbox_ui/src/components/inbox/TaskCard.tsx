...
/**
 * A single info chip to display on a card.
 */
interface BusinessChip {
    label?: string;
    value: string;
    isPrimary?: boolean;
}
...
/**
 * Extract key business details per document type from the businessContext
 * that the backend has enriched at list level.
 */
        ...
        if (ctx.type === 'PO' && ctx.po) {
            ...
            if (hdr) {
                if (hdr.purchaseOrderNetAmount) {
                    ...
                    chips.push({ label: 'Total', value: `${formatAmount(hdr.purchaseOrderNetAmount)} ${cur}`.trim(), isPrimary: true });
                }
            ...
            if (hdr) {
                ...
                if (totalValue) {
                    chips.push({ label: 'Total', value: `${formatAmount(totalValue)} ${totalCurrency}`.trim(), isPrimary: true });
                }
...
function PriorityBadge({ priority }: { priority?: string }) {
    ...
    const config: Record<
        string,
        { variant: 'destructive' | 'warning' | 'secondary' | 'outline'; label: string }
    > = {
    ...
    return (
        <Badge variant={variant} className="px-2 py-0.5 text-[11px] font-semibold">
            {label}
        </Badge>
    );
}
...
function StatusBadge({ status }: { status: string }) {
    const config: Record<
        string,
        { variant: 'success' | 'info' | 'secondary'; label: string }
    > = {
