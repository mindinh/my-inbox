...
import { executeTaskDetailBatch } from './task-detail-batch';
...
export interface ISapTaskClient {
    ...
    fetchTaskDetailBundle(sapUser: string, instanceId: string, userJwt?: string): Promise<SapTaskRaw>;
    ...
    /** Claim a task */
    claimTask(sapUser: string, instanceId: string, userJwt?: string): Promise<void>;

    /** Release a claimed task */
    releaseTask(sapUser: string, instanceId: string, userJwt?: string): Promise<void>;
...
export class SapTaskClient implements ISapTaskClient {
    ...
    constructor() {
        this.useDestination = process.env.SAP_USE_DESTINATION !== 'false';
    ...
    async fetchTaskDetailBundle(
        ...
        task.Comments = { results: batchData.comments };
        task.Attachments = { results: batchData.attachments };
        task.ProcessingLogs = { results: batchData.processingLogs };
        task.WorkflowLogs = { results: batchData.workflowLogs };
        task.DecisionOptions = { results: batchData.decisionOptions };
        if (batchData.commentsCount != null) {
            task.HasComments = batchData.commentsCount > 0;
        }
        if ((batchData.comments?.length || 0) > 0) {
            task.HasComments = true;
        }
        if (batchData.attachmentsCount != null) {
            task.HasAttachments = batchData.attachmentsCount > 0;
        }
    ...
    async claimTask(sapUser: string, instanceId: string, userJwt?: string): Promise<void> {
        const effectiveSapUser = this.resolveSapUser(sapUser);
        const origin = await this.resolveOriginForTask(effectiveSapUser, instanceId, userJwt);
        const url = `/Claim?InstanceID='${encodeURIComponent(instanceId)}'&SAP__Origin='${encodeURIComponent(origin)}'`;
        await this.postWithCsrf(effectiveSapUser, url, {}, userJwt);
    }

    async releaseTask(sapUser: string, instanceId: string, userJwt?: string): Promise<void> {
        const effectiveSapUser = this.resolveSapUser(sapUser);
        const origin = await this.resolveOriginForTask(effectiveSapUser, instanceId, userJwt);
        const url = `/Release?InstanceID='${encodeURIComponent(instanceId)}'&SAP__Origin='${encodeURIComponent(origin)}'`;
        await this.postWithCsrf(effectiveSapUser, url, {}, userJwt);
    }
